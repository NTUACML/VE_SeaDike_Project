...
bool VE_SD::Module1::OutPutLogFile(String ^ Pois)
{
	...
	if (Var->BasementCheckCondi == true) {
		FILE << "******地盤反力及乘載力檢核檢核******" << std::endl;
		FILE << "土壤凝聚力: " << Var->C << std::endl;
		FILE << "內摩擦角: " << Var->CentAngle << std::endl;
		FILE << "乘載力係數Nc: " << Var->Nc << std::endl;
		FILE << "乘載力係數Nq: " << Var->Nq << std::endl;
		FILE << "乘載力係數Nr: " << Var->Nr << std::endl;
		FILE << "作用力V: " << Var->V << std::endl;
		...
		FILE << "堤底作用力寬: " << Var->B_plum << std::endl;
		FILE << "最大反力: " << Var->Base_P1 << std::endl;
		FILE << "最小反力: " << Var->Base_P2 << std::endl;
		FILE << "拋石單位重: " << Var->BaseDen << std::endl;
		FILE << "入土深度: " << Var->U << std::endl;
		FILE << "拋石厚度: " << Var->D << std::endl;
		FILE << "Df: " << Var->Df << std::endl;
		FILE << "反力Angle: " << Var->Base_Theta << std::endl;
		FILE << "基礎作用力寬: " << Var->B_plum2 << std::endl;
		FILE << "最大反力: " << Var->R1 << std::endl;
		FILE << "最小反力: " << Var->R2 << std::endl;
		FILE << "極限乘載力: " << Var->Qu << std::endl;
		FILE << "容許乘載力: " << Var->Qa << std::endl;
		FILE << "安全係數: " << Var->BaseFS << std::endl;
	}

	//FILE << "測試: " << Var->Temp << std::endl;
	FILE.close();
	return true;
}
